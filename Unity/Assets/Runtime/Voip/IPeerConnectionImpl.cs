...
namespace Ubiq.Voip.Implementations
{
    ...
    public interface IPeerConnectionImpl : IDisposable
    {
        ...
        // IVoipAudioSource audioSource { get; }
        // IVoipAudioSink audioSink { get; }

        void Setup(VoipPeerConnection context,
            bool polite,
            List<IceServerDetails> iceServers);

        void ProcessSignallingMessage (SignallingMessage message);

        void SetRemotePeerRelativePosition (Vector3 position, Quaternion rotation);

        // todo
        VoipAudioSourceOutput.Stats GetStats ();
    }
